...
size_t String::find(const char* substring) const {
    ...
    for(size_t i = 0; i < length_ - strlen(substring); i++) {
        if (&str_[i] == pos) {
            return i;
        }
    }
...
size_t String::rfind(const char* substring) const {
    char* pos = strstr(str_, substring);
    size_t i0 = 0;
    while(strstr(&str_[i0], substring) != nullptr && i0 < length_) {
        pos = strstr(&str_[i0++], substring);
    }
    for(size_t i = 0; i < length_ - strlen(substring); i++) {
        if (&str_[i] == pos) {
            return i;
...
String String::substr(size_t start, size_t count) const {
    ...
    strncpy(result, &str_[start], count);
